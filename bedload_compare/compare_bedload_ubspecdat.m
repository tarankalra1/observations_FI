...
% MPM, vandera with ubspecfun (UBSPECDAT), vandera with averaged waveformfrom
...
load('/media/taran/DATADRIVE2/Obs_data/matfiles/vandera_bedld_workhorse_ubspecdat.mat','bedldx_wh_vspec','R','Beta','Ur')
...
legend('emp. waveform vand','mpm','Location','Northeast')
xlim([dn(nt1) dn(nt2)]);
datetick('x',2) % 
 print -dpng '../pngfiles/bedload/bedload_vand_vspec_mpm.png'
 
figure(3) 
% plot(dn(nt1:nt2),cumbedld_directwaveform(nt1:nt2)*dt,'k') 
% hold on
plot(dn(nt1:nt2),0.1*cumbedld_empirical_vand(nt1:nt2)*dt,'r'); 
hold on 
plot(dn(nt1:nt2),cumbedld_mpm(nt1:nt2)*dt,'b') 
legend('emp. waveform vand fac=0.1','mpm','Location','Northwest')
%legend('direct waveform vand','emp. waveform vand','mpm','Location','Northwest')
xlim([dn(nt1) dn(nt2)]);
datetick('x',2) % 
 print -dpng '../pngfiles/bedload/cumulative_bedload_vand_vspecfac_mpm_3600.png'
