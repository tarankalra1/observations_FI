function hand=wnormplot_crs(x,nr1,nr2,df)
% WNORMPLOT_CRS Plots data on a Normal distribution paper
% (Only diff is that this returns a handle to the symbols)
...
for i=1:m
  ...
  data(1,i)=mx(i);
  data(2,i)=sx(i)^2;
  hand=plot(x(:,i),Y,'b.','markersize',12);
  hold on
  if nr1
...
if nr2
  ...
  for l=1:length(levels)
    % figtext is from WAFO toolbox
    % h=figtext(1.01,lev(l),[num2str(levels(l)*100) '%'],'norm','data'); %
    h=text(1.01,lev(l),[num2str(levels(l)*100) '%']);
